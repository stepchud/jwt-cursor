...
export class JWTDecoder {
  private static readonly DEFAULT_CLOCK_SKEW = 30; // 30 seconds
   ...
   */
  public static decode(token: string): JWT {
   ...
   */
  public static decodeAndValidate(token: string, options: JWTValidationOptions = {}): JWTDecodeResult {
   ...
   */
  public static validate(jwt: JWT, options: JWTValidationOptions = {}): Omit<JWTDecodeResult, 'jwt'> {
    const errors: string[] = [];
   ...
   */
  public static getPayload(token: string): JWTPayload {
    const jwt = this.decode(token);
    return jwt.payload;
  }
   ...
   */
  public static getHeader(token: string): JWTHeader {
    const jwt = this.decode(token);
    return jwt.header;
  }
   ...
   */
  public static isExpired(token: string, clockSkew: number = this.DEFAULT_CLOCK_SKEW): boolean {
    try {
      const jwt = this.decode(token);
      if (!jwt.payload.exp) {
   ...
   */
  public static getTimeUntilExpiration(token: string): number | null {
    try {
      const jwt = this.decode(token);
      if (!jwt.payload.exp) {
        return null;
